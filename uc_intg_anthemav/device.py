...
class AnthemDevice(PersistentConnectionDevice):
    ...
    async def establish_connection(self) -> bool:
        ...
        try:
            _LOG.info(f"Connecting to {self.device_config.name} at {self.device_config.host}:{self.device_config.port}")
            ...
            _LOG.info(f"TCP connection established to {self.device_config.name} - Peer: {peer}")
            ...
            _LOG.info("Enabled Standby IP Control (SIP1) - device will respond in standby")
            await asyncio.sleep(0.1)
    ...
    async def maintain_connection(self) -> None:
        ...
        while self.is_connected and self._reader:
            try:
                ...
                if not data:
                    _LOG.warning(f"Connection closed by {self.device_config.name} (empty read)")
                    break
        ...
        _LOG.info(f"Receive loop ended for {self.device_config.name} (received {receive_count} messages total)")
        ...
        if response.startswith("IDM"):
            ...
            if input_match:
                ...
                if len(self._input_names) >= self._input_count:
                    ...
                    _LOG.info(f"Input name discovery complete: {len(self._input_names)} inputs")
                    self.events.emit(DeviceEvents.UPDATE, self.device_config.identifier, {"inputs_discovered": True})
